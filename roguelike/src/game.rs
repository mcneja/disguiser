...
pub fn on_draw(game: &Game, screen_size_x: i32, screen_size_y: i32) {
	...
	put_tile(208, game.player_position.0, game.player_position.1, gray);
...
pub fn on_key_down(game: &mut Game, key: i32, ctrl_key_down: bool, shift_key_down: bool) {
	...
	let new_position = (
		max(0, min(game.size_x - 1, game.player_position.0 + modifier.0)),
		max(0, min(game.size_y - 1, game.player_position.1 + modifier.1))
	);

	if new_position != game.player_position {
